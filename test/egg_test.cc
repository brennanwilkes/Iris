...
AudioSound* soi = AM -> get_sound("Assets/soi.wav");
...
AsyncTask::DoneStatus boiii(GenericAsyncTask* task, void* data);
...
int main(int argc, char *argv[]) {
	...
	NodePath scene = window->load_model(framework.get_models(), "./Stage/TESTSTAGE.egg");
	scene.reparent_to(window->get_render());
	scene.set_scale(5);
	scene.set_pos(-8, 42, 0);
	
	// Load our panda
	NodePath pandaActor = window->load_model(framework.get_models(), "./Kanon/untitled.egg");
	pandaActor.set_scale(1.8);
	pandaActor.set_pos(0, 0, 0);
	pandaActor.set_hpr(0, -90, 0);
 ...
 */
	...
	taskMgr -> add(new GenericAsyncTask("boiii", &boiii, (void*) &pandaActor));
...
AsyncTask::DoneStatus boiii(GenericAsyncTask* task, void* data){
	NodePath* actor = static_cast<NodePath*>(data);
	actor -> set_hpr(actor -> get_hpr().get_x() + 5, actor -> get_hpr().get_y(), actor -> get_hpr().get_z());
	return AsyncTask::DS_cont;
}
